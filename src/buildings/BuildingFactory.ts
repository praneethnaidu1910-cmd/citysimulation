...
import { GridPosition, GridPositionImpl } from '../core/GridPosition.js';
import { PowerConsumerComponent, TrafficGeneratorComponent, TaxGeneratorComponent } from '../core/Component.js';
import { BuildingInfoComponent, BuildingTemplate, BUILDING_TEMPLATES, ZoneType } from './BuildingTypes.js';
...
export class BuildingFactory {
  ...
  createBuilding(templateKey: string, position: GridPosition, zoneType?: ZoneType): Entity | null {
    ...
    const buildingPosition = new GridPositionImpl(
      position.x,
      position.y,
      template.width,
      template.height
    ...
    const buildingInfo = new BuildingInfoComponent(
      template.type,
      template.name,
      1, // Starting level
      3, // Max level
      template.constructionCost,
    ...
    const powerConsumer = new PowerConsumerComponent(
      Math.abs(template.powerRequired), // Use absolute value for consumption
    ...
    const trafficGenerator = new TrafficGeneratorComponent(
      template.dailyTrips,
      this.getTrafficMultiplier(template.type),
      [] // Destinations will be calculated by traffic system
    );
    entity.addComponent(trafficGenerator);

    // Add tax generator component
    const taxGenerator = new TaxGeneratorComponent(
      template.taxRevenue,
      this.getTaxRate(template.type),
      this.calculateLandValue(position, template)
    );
  ...
  private getPowerPriority(buildingType: string): number {
    switch (buildingType) {
  ...
  private getTrafficMultiplier(buildingType: string): number {
    switch (buildingType) {
  ...
  private getTaxRate(buildingType: string): number {
    switch (buildingType) {
  ...
  private calculateLandValue(position: GridPosition, template: BuildingTemplate): number {
  ...
  private getLandValueMultiplier(buildingType: string): number {
    switch (buildingType) {
  ...
  private updateComponentsAfterUpgrade(entity: Entity, oldLevel: number, newLevel: number): void {
    ...
    const trafficGenerator = entity.getComponent<TrafficGeneratorComponent>('TrafficGenerator');
    if (trafficGenerator) {
