...
export class CrimeSystem extends System {
    ...
    calculateCrimeScore(area, entities) {
    ...
    getBuildingCrimeFactor(buildingInfo) {
    ...
    calculatePolicePresence(area) {
        ...
        for (const policeStation of this.policeStations) {
            ...
            if (distance <= maxEffectiveDistance) {
                const buildingInfo = policeStation.getComponent('BuildingInfo');
    ...
    determineCrimeTypes(crimeScore, area, entities) {
        ...
        if (crimeScore > 0.2) {
            if (hasCommercial)
                types.push(CrimeType.PETTY_THEFT);
            if (hasResidential)
                types.push(CrimeType.BURGLARY);
            if (crimeScore > 0.5)
                types.push(CrimeType.VANDALISM);
            if (crimeScore > 0.7)
                types.push(CrimeType.ASSAULT);
            if (hasIndustrial && crimeScore > 0.4)
                types.push(CrimeType.DRUG_RELATED);
        }
        return types;
    }
