...
export class RenderEngine {
    ...
    setTrafficSystem(trafficSystem) {
        this.trafficSystem = trafficSystem;
    }
    toggleZoneDisplay() {
        this.showZones = !this.showZones;
    }
    toggleLandValueDisplay() {
        ...
        for (let gridX = startGridX; gridX <= endGridX; gridX++) {
            for (let gridY = startGridY; gridY <= endGridY; gridY++) {
                ...
                if (this.camera.zoom > 1.0) {
                    ...
                    this.ctx.fillText(`$${Math.floor(value / 1000)}k`, screenX + screenSize / 2, screenY + screenSize / 2);
                }
            }
        }
    }
    ...
    drawEntity(entity) {
        ...
        if (this.camera.zoom > 0.5) {
            ...
            this.ctx.textAlign = 'center';
            this.ctx.fillText(entity.id.substring(0, 8), screenX + screenWidth / 2, screenY + screenHeight / 2);
        }
    }
    ...
    drawRoadEdge(edge, roadNetwork) {
        ...
        if (!this.isLineVisible(startScreenPos, endScreenPos))
            return;
    ...
    drawRoadNode(node) {
        ...
        if (!this.isPointVisible(screenPos))
            return;
    ...
    drawTrafficFlow(start, end, density) {
    ...
    getTrafficDensityColor(density) {
        if (density < 0.3)
            return '#27AE60'; // Green - light traffic
        if (density < 0.6)
            return '#F39C12'; // Orange - moderate traffic
        if (density < 0.8)
            return '#E67E22'; // Dark orange - heavy traffic
